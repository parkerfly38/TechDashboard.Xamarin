...
namespace TechDashboard.Data
{
    public partial class TechDashboardDatabase
    {
        ...
        public List<JT_MiscellaneousCodes> GetMiscellaneousCodesFromDB(string codeType)
        {
            lock (_locker)
            {
                // todo:  miscellaneous codeType may need more than just *E, need clarification
        ...
        public CI_Item GetUnitOfMeasureforCode(string code)
        {
            string expensePermutation = @"/" + code;

            return _database.Table<CI_Item>().Where(x => x.ItemCode == code || x.ItemCode == expensePermutation).FirstOrDefault<CI_Item>();
        }
        ...
        public List<JT_MiscellaneousCodes> GetAllWorkTicketStatusesFromDB()
        {
            lock (_locker)
            {
                return GetMiscellaneousCodesFromDB();
            }
        }
        ...
        /// <summary>
        /// retrieves only one character earning codes, for clockout screen
        /// </summary>
        /// <returns></returns>
        public List<JT_EarningsCode> GetOneCharEarningsCodesFromDB()
        {
            return GetEarningsCodesFromDB().Where(code => code.EarningsCode.Length == 1).ToList();
        }
