...
namespace TechDashboard.Data
{
    ...
    public partial class TechDashboardDatabase
    {
        ...
        public void FillCustomerContactTable()
        {
            ...
            /*List<SO_SalesOrderHeader> salesOrders = GetSalesOrderHeadersFromDB().GroupBy(soh => soh.CustomerNo).Select(soh => soh.First()).ToList();
            /for (int i = 0; i < salesOrders.Count; i++)
            {
                if (i > 0)
                {
            ...
            // bk on a lot of these, the post of filters is too voluminous - will need to revisit after stabilized
            FillLocalTable<AR_CustomerContact>();
